...
if (!function_exists('array_remap')) {
    function array_remap($parameters, $rules, $record = [])
    {
        if (is_object($record)) {
...
if (!function_exists('array_indexing_by_key')) {
    function array_indexing_by_key($key, $array)
    {
        $new_array = array();
        if (empty($array)) {
            return $array;
...
if (!function_exists('array_filter_by_keys')) {
    ...
    {
        $new_array = array_filter($array, function ($var) use ($keys) {
            return array_search($var, $keys) !== FALSE;
...
/**
 * 連想配列の指定したキーの中身を合計します
 */
if (!function_exists('array_assoc_total')) {
    function array_assoc_total($array, $key)
    {
        $total = 0;
...
if (!function_exists('array_selector')) {
    function array_selector($key, $array, $default = "", $subkey = "")
    {
        if (is_null($key) || $key === "" || $key === []) {
            $result = $default;
        } else if (is_object($array) && isset($array->{$key})) {
            $result = $array->{$key};
            if ($subkey !== "") {
