...
class CustSeeder extends BaseSeeder
{
    ...
    protected function insertRecord($record = []) {
            ...
            $dao = Cust::where("name",$record["name"])->first();
            if (is_null($dao)) {
                $dao = new Cust();
            }
            ...
            if (count($tenpoModels)) {
                ...
                if (isset($assign["schedule"])) {
                    ...
                    if ($notInTenpos->isNotEmpty()) {
                        ...
                        $schdao = Schedule::where("sc_cid",$dao->cid)->where("sc_memtype",$dao->memtype)->where("sc_tenpo",$changeTenpo->tid)->first();
                        if (is_null($schdao)) {
                            $schdao = new Schedule();
                        }
                    ...
                    if ($notInMemtype->isNotEmpty()) {
                        ...
                        $schdao = Schedule::where("sc_cid",$dao->cid)->where("sc_memtype",$changeMemType->mid)->where("sc_tenpo",$thisTenpo->tid)->first();
                        if (is_null($schdao)) {
                            $schdao = new Schedule();
                        }
                        $assign["schedule"]["sc_cid"]     = $dao->cid;           // 会員ID
